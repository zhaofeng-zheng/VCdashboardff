...
vcfiles<-list.files(path = "/home/ffoley/ShinyApps/VCCubs_OM_Test", pattern = "*.csv", full.names = T)

for(file in vcfiles) 
{
filename <- str_replace_all(file, "/home/ffoley/ShinyApps/VCCubs_OM_Test/", "")
...
#-------Margins line Plot ----------------------------------------------- 
  ...
  yselected <- reactiveVal()
  
  
  observeEvent(input$marginType, {
    if (input$marginType != "Actual_margin_per_min") {
      yselected(paste0(input$marginType, "_", input$marginCalcType))
    } else{
      yselected(input$marginType)
    }
  })
  ...
  marginBreakdownData <- reactive({
    VCA_Margins %>%
      ...
      mutate(Actual_margin_per_min = Actual_margin_sum / Duration_in_min_sum) %>%
      ...
      mutate(negMargin = !!sym(yselected()) < 0) %>%
      filter(!is.na(!!sym(input$viewBy))) %>%
      group_by(Start_month, negMargin) %>%
      arrange(desc(!!sym(yselected())), .by_group = TRUE) %>%

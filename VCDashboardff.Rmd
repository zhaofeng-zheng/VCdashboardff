...
vcfiles<-list.files(path = "D:/Work/data", pattern = "*.csv", full.names = T)

for(file in vcfiles) 
{
filename <- str_replace_all(file, "D:/Work/data/", "")
filename <- str_replace_all(filename, ".csv","")
filename <- str_replace_all(filename, "dbo.","")
filename <- str_replace_all(filename, " ","")
assign(filename,read.csv(file,header = TRUE, fileEncoding="UTF-8-BOM"))
print(filename)
}

# VCA_Margins
VCA_Margins$Client_item_id <- as.integer(VCA_Margins$Client_item_id)
VCA_Margins$Outlet_item_id <- as.integer(VCA_Margins$Outlet_item_id)
VCA_Margins <-
  left_join(VCA_Margins,
            VCA_Clients_view,
            by = c("Client_item_id" = "Client_item_id"))
VCA_Margins$Allocated_resource_item_id <-
  as.integer(as.character(VCA_Margins$Allocated_resource_item_id))
VCA_Margins$Temporary_allocated_resource_item_id <-
  as.integer(as.character(VCA_Margins$Temporary_allocated_resource_item_id))
VCA_Margins <-
  left_join(
    VCA_Margins,
    VCA_Workers_view %>% rename(initial_worker = Team_Member) %>% select(initial_worker, Resource_item_id),
    by = c("Allocated_resource_item_id" = "Resource_item_id")
  )
VCA_Margins <-
  left_join(
    VCA_Margins,
    VCA_Workers_view %>% rename(sub_worker = Team_Member) %>% select(sub_worker, Resource_item_id),
    by = c("Temporary_allocated_resource_item_id" = "Resource_item_id")
  )
VCA_Margins <-
  mutate(VCA_Margins,
         actual_worker = case_when(
           !is.na(sub_worker) ~ sub_worker,!is.na(initial_worker) ~ initial_worker
         ))
VCA_Margins$Outlet_item_id <-
  as.integer(as.character(VCA_Margins$Outlet_item_id))
VCA_Margins$Client <- VCA_Margins$Participant
VCA_Margins <-
  left_join(VCA_Margins,
            VCA_Outlets_View,
            by = c("Outlet_item_id" = "outlet_item_id"))
VCA_Margins <-
  left_join(
    VCA_Margins,
    VCA_Outlets_View %>% rename(service_delivery_location_name = outlet_name) %>% select(service_delivery_location_name, outlet_item_id),
    by = c("Outlet_item_id" = "outlet_item_id")
  )
 ...
 #####################-Clients-#####################################################------------   
   ...
   bs4Card(  id = "OurClients",  # type = 1,   imageElevation = 4,  # height = "40px",
         ...
         bs4Card( width = 12,  title= span("Client Retention ", style ="font-size: 25px; color: black;" ), elevation = 5,       
           plotlyOutput("ClientRetentionAging",  height = "500px" ))  , 
         br(),
         br(),
         bs4Card( width = 12,  title= span("Client Retention ", style ="font-size: 25px; color: black;" ), elevation = 5,       
...
#####################-Sustainability-#####################################################------------   
   ...
   bs4Card(  id = "OurBusiness",   type = 1,   imageElevation = 4,  # height = "40px",
          ...
          bs4DashControlbar (   br(),  br(),  br(),  br(),  br(), br(),
              ...
              disable = FALSE,width = 250,skin = "light", status = "white",
            elevation = 4,   collapsed = FALSE ,  minified = TRUE,  expandOnHover = TRUE, startOpen = TRUE),
          # Devin: marginBreakdown (stacked bar + datatable)
          bs4Card(
            width = 12,
            title = span("Margin breakdown by outlet, client or worker",
                         style = "font-size: 25px; color: black;"),
            elevation = 5,
            shinyjs::useShinyjs(),
            plotlyOutput("marginBreakdown",  height = "550px"),
            conditionalPanel("input.showData==true",
            ...
            selectInput(
              ...
              choices = c(
                ...
                "Approved Margin" = "Approved_margin",
                "Actual Margin per Min" = "Actual_margin_per_min"
              ),
              selected = "Actual_margin"
            ),
            conditionalPanel(
              "input.marginType != 'Actual_margin_per_min'",
              selectInput(
                "marginCalcType",
                "Mean / Sum",
                choices = c("Mean" = "mean",
            ...
            selectInput(
              "viewBy",
              "View by Outlet, Client or Worker?",
              choices = viewByChoices,
              selected = "outlet_name"
            ),
            checkboxInput("showData", "Show Datatable"),
            conditionalPanel(
              "input.showData==true",
              # dateRangeInput("date", "Date", servicemonthmin, servicemonthmax, servicemonthmin, servicemonthmax, format="dd-mm-yyyy"),
              uiOutput("date"),
              selectInput(
                "outlet_name",
                "Outlet Name",
                choices = levels(factor(VCA_Margins$outlet_name)),
                selectize = TRUE,
                multiple = TRUE
              ),
              selectInput(
                "client",
                "Client",
                choices = levels(factor(VCA_Margins$Client)),
                selectize = TRUE,
                multiple = TRUE
              ),
              selectInput(
                "worker",
                "Worker",
                choices = levels(factor(VCA_Margins$actual_worker)),
                selectize = TRUE,
                multiple = TRUE
              )
            )
...
#------Clients Transform input values Filter, group and summarise using the transformed input values ---------------
  ...
  filtered_plot <- reactive({
    VCA_Clients_actuals_monthly %>%
      filter (
        #outlet_name  %in%  thelocs() |  thelocs()  == 'all' ,
  ...
  filtered_plot_pie <- reactive({
    VCA_Clients_actuals_monthly %>%   filter(
      Client  %in%  input$SelectClient ,
      outlet_name %in% input$SelectOutlet
      ,
      between (Service_month, input$Timeframe[1], input$Timeframe[2])
    )    %>%
...
#-------Clients Demographics bar Plot ----------------------------------------------- 
   myColours <- c(
     "#c9daf2", "#b8d7f4", "#b0daf7", "#a4dafa", "#97dafc", "#8ecffc", "#84c3fb", "#7bb7fb",       "#76b1fb", "#71abfa",
     "#669fee", "#5b92e2", "#5086d6", "#4479ca", "#2d60b2", "#2253a6", "#16469a",
     "#244792", "#324889", "#404981", "#47497d", "#4e4978"
   )
    ...
    output$Demographics <- renderPlotly({
      ggplotly(
        ggplot(
          ...
          scale_fill_manual(name = " ", values = myColours) +
          theme(
    ...
    output$Demographicspie <- renderPlotly({
      ...
      plot_ly(
        ...
        layout(
          ...
          legend = list(
            title = list(text =  names(choicel[choicel == lselected()])),
            legend.position = "right",
            legend.key.width = unit(4, 'cm'),
            legend.title.alignment = "right"
          )
        )
    })
...
#--------Client Cohort Analysis % over time  ---Plot------  
    ...
    output$ClientRetentionAging <- renderPlotly({
      Activebymonth <-
        VCA_Clients_actuals_monthly   %>%
      ...
      Activebymonthplot <-
        data.frame(Activebymonth = Activebymonth$first,
                   check.rows = FALSE,
                   t(apply(
                     select(as.data.frame(Activebymonth), 2:ncol(Activebymonth)), 1,  shiftrow
                   )))
      colnames(Activebymonthplot) <-
        c("Client_retention", sub("", "Month ",
                                  str_pad(1:(
                                    ncol(Activebymonthplot) - 1
                                  ), 2, pad = "0")))
      
      #--- and make column names readable # first should be "cohort" and the rest week.<number>, (padded)----------
      colnames(Activebymonthplot) <-
        c("Client_retention", sub("", "Month ", str_pad(1:(
          ncol(Activebymonthplot) - 1
        ), 2, pad = "0")))
      ...
      Client_retention_perc <-
        data.frame(
          Client_retention = Activebymonthplot$Client_retention,
          check.rows = FALSE,
      ...
      Activebymonthplot_abs <-
        gather(Activebymonthplot,
               "Retention_age",
               "Client_Count"  ,
               2:ncol(Activebymonthplot))   # })
      labelnames <-
        c(
          Activebymonthplot_abs$Client_Count[1:(ncol(Activebymonthplot) - 1)],
      ...
      Activebymonthplotdata <- data.frame(
        ...
        Client_retention_percentage = Activebymonthplot_pct$Percentage_of_Clients,
        check.rows = FALSE
      )
      ...
      ggplotly(
        ggplot(
          Activebymonthplotdata ,
          aes(x = Client_retention_age,
              y = reorder(
                Client_retention, desc(Client_retention)
              ),
              text=paste0(Client_retention_age, " after ", Client_retention,
                          "\nRetention percent ", Client_retention_percentage))
        ) +
          geom_tile(aes(fill = Client_retention_percentage)) +
          scale_fill_gradient(
            low = "#4e4978",
            high = "#c9daf2"
          ...
          labs(x = "Clients retained 'x' months after commencement", y =
                 "Client commencement month end ") +
          ggtitle(
            paste(
              "Client retention aging and % ( first column is number of new Clients each month ) "
            )
          ) +
          theme(
...
ggplotly(
  Activebymonthplot_pct[!is.na(Activebymonthplot_pct),] %>%
    mutate(Client_retention = factor(Client_retention)) %>%
    ggplot(
      aes(
        x = Retention_age,
        y = Percentage_of_Clients,
        group = Client_retention,
        color = Client_retention,
        text = paste(
          '</br>Retained in: ',
          Retention_age,
          '</br>Commenced in: ',
    ...
    labs(
      title = "Cohorts Retention ratio", 
      color = "The Month Client Commenced Services",
      x="Client still active X months after Client commenced services",
      y="% retained"
    ) +
...
#------------unalloc   Octopus collapsible tree------#  https://adeelk93.github.io/collapsibleTree/  extras 
 ...
 ot <- reactive ({
   VCA_Unallocated_Shifts %>%
     mutate (
       Estimated_start =   as_datetime(Estimated_start) ,
       Suburb = toupper(Suburb),
       Funding_type_name =   substr(Funding_type_name, 1, 20) ,
       Estimated_end = as_datetime(Estimated_end),
       Start_date =  ymd(Start_date)  ,
       Start_month = floor_date(Start_date, 'month') ,
       Nodesize = Shift_hours / 100.0
     )    %>%
     filter (Funding_type_name != "002-Transport (auto payments)") %>%   #, Suburb %in% input$SelectSuburbU ,
     ...
     mutate("Shift hours" = Shift_hours)
 }) 
 ...
 output$Unallocateddemandtree <- renderCollapsibleTree (
   collapsibleTree(
     ...
     root = deparse(substitute("Unallocated Demand")),
     inputId = NULL,
     attribute =  "Shift hours"  ,
     aggFun = sum,
     fill = "lightsteelblue",
     fillByLevel = FALSE ,
     linkLength = NULL,
   ...
   #                            ID1[click_data()$pointNumber + 1,1]    else ID1[1:nrow(ID1),1]  } ) 
     ...
     # input$thedowname <- reactive ({ "All"  })  
      color_index <- data.frame(
        cols = c("#a6000e", "#b33702" , "#f7c200" , "#e7f700",  "#36f700",  "red"),
...
output$safetyweekdayp <-
 renderPlot({
   (
     ggplot(
       VCA_Incidents %>%  #filter( Incident_weekday  %in%  thefilter() )  %>%
         mutate(
           Incident_weekday,
           Incidents = 1,
           Weekdayno = lubridate::wday(dmy(Incident_date), week_start = 1)
         )    %>%
         ...
         merge(
           color_index,
           by.x = "Incident_severity",
           by.y = "labels",
           all.x = TRUE
         )   %>%
         arrange(Weekdayno, Incident_severity)  %>%
         mutate(Incident_severity = factor(
           Incident_severity,
           levels =  c(
       ...
       geom_bar(aes(
         x = reorder(Incident_weekday, Weekdayno),
         y = Incidents,
         fill = Incident_severity
       ), stat = "sum") +
...
output$safetyclock <-  renderPlot ({
  ...
  col_direction =   c(
    "1 Insignificant" = "#90ee90",
    "2 Minor"  = "#228B22",
    "3 Moderate" = "yellow",
    "none" = "white",
    "4 Major" = "#FF9F00",
    "5 Reportable Incident" = "red"
  )
  maxy <-
    max(TheHours %>% group_by (Incident_hour) %>%  summarise_at(vars(Incidents), .funs =
                                                                  "sum"))
  TheHoursmatrix <-
    as.matrix(
      TheHours  %>%  dplyr::select (Incident_hour, Incident_severity , Incidents) %>%
  ...
  arrows(0,
         0,
         cos(hour.degree / 180 * pi) * 0.3,
         sin(hour.degree / 180 * pi) * 0.3,
         lwd = 3)
  ...
  legend(
    ...
    fill = c("#36f700" , "#e7f700" , "#f79000",  "#f75b00",  "#f70000"), 
    border = NULL,
...
output$safetyweekdaytype <- renderPlotly ({ 
     cols <- c( " _ Unknown" = "#d3d3d3", "1 Insignificant" = "#FF9F00", "2 Minor"  = "yellow",  
            "3 Moderate" = "#228B22",   "4 Major" = "#90ee90", "5 Reportable Incident"  = "#d3d3d3" )
...
#-------Margins line Plot ----------------------------------------------- 
  ...
  #-------------- Margin breakdown (Devin) ----------------------------
  # what is the stacked bar based on? Actual margin ? Estimated? or margin earned per min?
  ySelected_Margin <- reactiveVal()


  observeEvent(input$marginType, {
    if (input$marginType != "Actual_margin_per_min") {
      ySelected_Margin(paste0(input$marginType, "_", input$marginCalcType))
    } else{
      ySelected_Margin(input$marginType)
    }
  })

  observeEvent(input$viewBy, {
    toggleState("outlet_name", condition = input$viewBy == "outlet_name")
    toggleState("client", condition = input$viewBy == "Client")
    toggleState("worker", condition = input$viewBy == "worker")
  })

  marginBreakdownData <- reactive({
    VCA_Margins %>%
      select(
        Client,
        item_id,
        item_type_cd,
        Start_month,
        Allocated_resource_item_id,
        Client_item_id,
        Outlet_item_id,
        Case_Item_id,
        Temporary_allocated_resource_item_id,
        Estimated_client_revenue,
        Actual_client_revenue,
        Approved_client_revenue,
        Estimated_resource_cost,
        Actual_resource_cost,
        Approved_resource_cost,
        Worker_Base_Cost,
        Service_Delivery_Location,
        service_delivery_location_name,
        Duration_in_min,
        uom_cd,
        unit_price_cy,
        deliv_outlet_item_id,
        initial_worker,
        sub_worker,
        actual_worker,
        outlet_name
      ) %>%
      mutate(Start_month = dmy(Start_month)) %>%
      mutate(Start_month = ym(paste0(
        year(Start_month), "/", month(Start_month)
      ))) %>%
      mutate(
        Estimated_margin = Estimated_client_revenue - Estimated_resource_cost,
        Actual_margin = Actual_client_revenue - Actual_resource_cost - Worker_Base_Cost,
        Approved_margin = Approved_client_revenue - Approved_resource_cost
      ) %>%
      mutate(no_Clients = 1) %>%
      mutate(no_Workers = 1) %>%
      group_by(Start_month,!!sym(input$viewBy)) %>%
      summarise(across(where(is.numeric), list(
        mean =  ~ mean(., na.rm = TRUE),
        sum =  ~ sum(., na.rm = TRUE)
      )), .groups = "keep") %>%
      mutate(Actual_margin_per_min = if(exists("Actual_margin_sum")){Actual_margin_sum}else 1 / Duration_in_min_sum) %>%
      filter(!is.infinite(Actual_margin_per_min)) %>%
      mutate(
        across(where(is.numeric) & !Actual_margin_per_min, round, 2),
        Actual_margin_per_min = round(Actual_margin_per_min, 5)
      ) %>%
      mutate(negMargin = !!sym(ySelected_Margin()) < 0) %>%
      filter(!is.na(!!sym(input$viewBy))) %>%
      group_by(Start_month, negMargin) %>%
      arrange(desc(!!sym(ySelected_Margin())), .by_group = TRUE) %>%
      mutate(rank = case_when(negMargin == FALSE ~ n():1,
                              TRUE ~ -n():-1))
  })

  output$date <- renderUI({
    servicemonthmin <- min(ymd(marginBreakdownData()$Start_month))
    servicemonthmax <- max(ymd(marginBreakdownData()$Start_month))
    dateRangeInput(
      "date",
      "Date",
      servicemonthmin,
  ...
  output$dataTable <- renderDataTable({
    req(marginBreakdownData(), input$date)
    marginBreakdownData() %>%
      filter(Start_month >= input$date[1] &
               Start_month <= input$date[2]) %>%
      filter(!!sym(input$viewBy) %in% c(input$outlet_name, input$client, input$worker))
  })

  output$marginBreakdown <- renderPlotly({
    ggplotly(
      ggplot(
        marginBreakdownData(),
        aes(
          x = Start_month,
          y = !!sym(ySelected_Margin()),
          fill = ordered(rank),
          text = case_when(
            input$viewBy == "outlet_name" ~ paste0(
              "Start month: ",
              Start_month,
              "\nOutlet name: ",
              if (exists("outlet_name")) {
                outlet_name
              } else
                "",
              "\nTotal number of minutes worked: ",
              Duration_in_min_sum,
              "\nTotal no. of times clients are served in this region: ",
              no_Clients_sum,
              "\nActual margin per min in ",
              if (exists("outlet_name"))
                outlet_name,
              ": ",
              Actual_margin_per_min,
              "\nTotal actual margin: ",
              if(exists("Actual_margin_sum")){Actual_margin_sum}else "",
              "\nMargin ",
              input$marginCalcType,
              ": ",
              !!sym(ySelected_Margin())
            ),
            input$viewBy == "Client" ~ paste0(
              "Start month: ",
              Start_month,
              "\nClient name: ",
              if (exists("Client")) {
            ...
            input$viewBy == "actual_worker" ~ paste0(
              "Start month: ",
              Start_month,
              "\nWorker name: ",
              if (exists("actual_worker")) {
                actual_worker
              } else
                "",
        ...
        labs(
          x = "Start date",
          y = paste0("Sum of margin by ", names(viewByChoices)[viewByChoices == input$viewBy]),
          tooltip = "all"
        ) +
        scale_fill_manual(values = myColours) +
        theme(
        ...
        guides(fill = guide_legend(title = input$viewBy)),
      tooltip = "text"
    )
  })
